...
static int file_skip(struct output_file* out, int64_t cnt) {
  ...
  ret = _lseeki64(outn->fd, cnt, SEEK_CUR);
  if (ret < 0) {
    error_errno("lseek64");
    return -1;
  }
...
static int file_write(struct output_file* out, void* data, size_t len) {
  ...
  while (len > 0) {
    ret = _write(outn->fd, data, (unsigned int)len);
    if (ret < 0) {
      if (errno == EINTR) {
        continue;
      }
...
//
//  while (len > 0) {
//    ret = gzwrite(outgz->gz_fd, data, min(len, (size_t)INT_MAX));
//    if (ret == 0) {
...
static int callback_file_open(struct output_file* out, int) {
  return 0;
}
...
int read_all(int fd, void* buf, size_t len) {
  ...
  while (total < len) {
    ret = _read(fd, ptr, (unsigned int)(len - total));
...
static int output_file_init(struct output_file* out, int block_size, int64_t len, bool sparse,
  ...
  if (sparse) {
    sparse_header_t sparse_header = {
        ...
        CHUNK_HEADER_LEN,
        out->block_size,
        static_cast<unsigned>(DIV_ROUND_UP(out->len, out->block_size)),
...
int write_fd_chunk(struct output_file* out, unsigned int len, int fd, int64_t offset) {
  ...
  pos = _lseeki64(fd, offset, SEEK_SET);
  if (pos < 0) {
    free(data);
    return -errno;
  }
...
int write_file_chunk(struct output_file* out, unsigned int len, const char* file, int64_t offset) {
  ...
  int file_fd;
  _sopen_s( &file_fd, file, O_RDONLY | O_BINARY, _SH_DENYWR, _S_IREAD | _S_IWRITE );
  if (file_fd < 0) {
    return -errno;
  }
