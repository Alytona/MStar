...
class SparseFileFdSource : public SparseFileSource {
 ...
 public:
  ...
  void Seek( int64_t off ) override { _lseeki64( fd, off, SEEK_CUR); }

  int64_t GetOffset() override { return _lseeki64(fd, 0, SEEK_CUR); }

  int SetOffset(int64_t offset) override {
    return _lseeki64(fd, offset, SEEK_SET) == offset ? 0 : -errno;
  }
...
static int process_skip_chunk(struct sparse_file* s, unsigned int chunk_size,
                              SparseFileSource* source, unsigned int blocks,
