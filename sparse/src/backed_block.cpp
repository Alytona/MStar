...
void backed_block_list_move(struct backed_block_list* from, struct backed_block_list* to,
                            struct backed_block* start, struct backed_block* block_end) {
  ...
  if (!block_end) {
    for (block_end = start; block_end && block_end->next; block_end = block_end->next)
      ;
  }

  if (start == nullptr || block_end == nullptr) {
    return;
  }
  ...
  if (from->data_blocks == start) {
    from->data_blocks = block_end->next;
  } else {
    for (bb = from->data_blocks; bb; bb = bb->next) {
      if (bb->next == start) {
        bb->next = block_end->next;
        break;
      }
    }
  }
...
int backed_block_add_file(struct backed_block_list* bbl, const char* filename, int64_t offset,
  ...
  bb->file.filename = _strdup(filename);
